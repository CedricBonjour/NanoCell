...
edit(x,y,n){
  ...
  if(stg.autoRound){
    ...
    }catch(e){throw new Error("edit error n = "+n)}
...
undo(){
  ...
  }while(prev &&  action.t - prev.t< Matrix.MS_DELTA);
  this.isSaved = false;
}
redo(){
  var prev, action;
  do {
   ...
   }while(next && next.t - action.t < Matrix.MS_DELTA );
   this.isSaved = false;
}
square(){
    var m = 1;
...
Object.defineProperty(Matrix, 'MS_DELTA', {value: 100});
