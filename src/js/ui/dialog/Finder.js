...
find(force = false){
  ...
  if (this.search.length<1){
    ...
    var yEnd   = (this.advanced && this.rowBool.value)?this.rowNum.value-1:this.sheet.matrix.height-1;
    var xEnd   = (this.advanced && this.colBool.value)?this.colNum.value-1:this.sheet.matrix.width-1;
    for(var y = yStart; y <=yEnd;y++ )for(var x = xStart ; x <=xEnd; x++) {
          var v = this.sheet.matrix.get(x,y);
...
replaceAll(){
  ...
  for(var e of this.found){
    this.exp.lastIndex = 0;
    this.sheet.matrix.edit(e.x,e.y, e.v.replace(this.exp, this.replaceIn.value));
  }
  this.sheet.refresh();
  this.find(true);
}
